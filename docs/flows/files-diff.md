...
The per-file viewer over the same `Base...HEAD` patch [raw-diff.md](./raw-diff.md) shows as text.

## Trigger

The user opens **Inspection → Files diff**, refreshes while it is open, or changes Base. Inside the view: the layout toggle, a file's collapse control, a gap expander, and the file navigator.
...
3. `generate_files_diff` → `GitRepository::files_diff` → `inspection::diff::files_diff`, which parses the string `branch_diff` already returns. One `git diff` process, both surfaces.
...
- Git writes: none.
- Other side effects: none. No clipboard, no persistence — the layout and navigator choices live in `AppState` for the session only.
