...
4. The review renders beside the form, and focus moves to its title. The selection stays intact behind it.
5. Apply consumes the plan by id. Cancel — the button or Escape — releases it. Changing operation or section cancels first rather than abandoning the plan.
...
7. On failure the review is discarded, state is reloaded before the error is shown, and the error banner keeps the Git message verbatim.
