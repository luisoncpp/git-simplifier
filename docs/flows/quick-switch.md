...
7. Checkout moves:
   - existing local: `git switch --no-recurse-submodules --ignore-other-worktrees --no-guess -- <target>`
   - remote-only: `git switch --no-recurse-submodules --ignore-other-worktrees -c <local> <remote-ref>` then `branch.<local>.remote` / `.merge` are set so the remote is upstream
