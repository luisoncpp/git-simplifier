...
- Inspection rail with a stable, binary-capable `Base...HEAD` patch in two shapes: **Files diff**, a per-file viewer with line numbers, syntax highlighting, unified/side-by-side layouts, GitLab-style context expansion, and a collapsible file navigator; and **Raw diff**, the same patch as text with one-click clipboard copy. Both are parsed from a single Git invocation, so they cannot drift.
