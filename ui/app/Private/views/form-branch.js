...
import { pathSetFor } from "../draft.js";
...
export function splitBranchForm(state) {
  if (!baseRef(state)) {
...
function hasPartner(state, path) {
  const partner = path.endsWith(".meta") ? path.slice(0, -".meta".length) : `${path}.meta`;
  return state.paths.some((entry) => (entry.path?.value ?? entry.path) === partner && !state.draft.splitPaths.has(partner));
}
