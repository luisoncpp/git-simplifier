...
function settingsGear(state: AppState): string {
  const current = state.view === "settings";
  return `<button class="nav-item settings-gear${current ? " active" : ""}" type="button"
    data-event="set-view" data-value="settings" aria-label="Settings" title="Settings"
    aria-current="${current ? "page" : "false"}">
    <span class="settings-gear-label">
      <svg class="gear-icon" viewBox="0 0 24 24" width="15" height="15" aria-hidden="true">
